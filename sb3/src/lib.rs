        ...
        for (id, block) in &blocks {
            ...
            let script = match block_info.opcode {
                ...
                _ => structured::Script {
                    hat: None,
                    position,
                    body: parser.parse_stack(Some(id), None)?,
                },
            };

            if script.body.is_empty() && script.hat.is_none() {
                return Err(
...
impl<'a> RawTargetParser<'a> {
    ...
    fn parse_value_ref(
        ...
        match reference {
            ...
            raw::BlockArrayOrId::Array(raw::BlockArray::ColorOrString(_, string)) => {
                Ok(Value::Literal(structured::Literal::String(string.clone())))
            }
            raw::BlockArrayOrId::Array(raw::BlockArray::VariableOrList(_, _name, id, ..)) => {
                Ok(Value::Variable(self.lookup_variable(block_id, id)?))
            }
            raw::BlockArrayOrId::Array(raw::BlockArray::Broadcast(_, _name, id)) => {
                if let Ok(var) = self.lookup_variable(block_id, id) {
    ...
    fn lookup_procedure_argument_by_name(
        ...
        let procedure = current_procedure.ok_or_else(|| {
            structured::StructuredConversionError::UnknownProcedure {
                block_id: block_id.into(),
            ...
            Stmt::CallProcedure {
                    ...
                    .zip(procedure_arguments.iter())
                {
                    let input = match (argument, procedure_argument.kind) {
                        ...
                        (
                            structured::ProcedureInput::Predicate(predicate),
                            structured::ProcedureArgumentKind::Boolean,
                        ...
                        | (
                            structured::ProcedureInput::Predicate(_),
                            structured::ProcedureArgumentKind::StringOrNumber,
                        ) => {
                            return Err(structured::StructuredConversionError::InvalidLiteral {
                                block_id: id.clone(),
